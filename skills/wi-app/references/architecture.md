...
- Framework media Elements (`Image`, `Video`, `Iframe`, `Gallery`, `Swiper`) extend `class/Elements/Media/Media.php`; keep shared fluent capabilities such as `CanSpanColumn` on that base rather than duplicating them per media.
